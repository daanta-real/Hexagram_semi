...
if (request.getParameter("section") != null && !request.getParameter("section").equals("0")
		&& !request.getParameter("section").equals("")) {
...
if (request.getParameter("pages") != null && !request.getParameter("pages").equals("0")
		&& !request.getParameter("pages").equals("")) {
...
<body>
	<h1>
		<a href="./KakaoDB.jsp">관광지 Google</a>
	</h1>
		...
		<table border="1" style='width:1050px;'>
			...
			<tbody align="center">
				...
				<tr>
					...
					<td><a href="<%=detail%>"><img
							src="<%=list.get(i).getImg()%>" width="80" height="80"><br />클릭</a></td>
					<%
					...
					%>
					<td><a
						href="./KakaoDetail.jsp?address=<%=address%>&title=<%=title%>&mapy=<%=mapy%>&mapx=<%=mapx%>&img=<%=img%>&contentid=<%=contentid%>">상세
							보기(custom)</a></td>
	...
	<a
		href="./KakaoDB.jsp?region=<%=region%>&section=<%=section%>&pages=<%=pages%>&number=<%=number - 1%>">이전
		페이지</a> /
	<a
		href="./KakaoDB.jsp?region=<%=region%>&section=<%=section%>&pages=<%=pages%>&number=<%=number + 1%>">다음
		페이지</a>
	<% } else { %>
		<h2>더이상 항목이 없네요..</h2>
	<% } %>
