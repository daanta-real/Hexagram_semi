...
@WebFilter(urlPatterns = {
		"/event_reply/update.nogari",
		"/event_reply/delete.nogari"
})
public class EventReplySelfFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			...
			int eventReplyIdx  = Integer.parseInt(req.getParameter("eventReplyIdx"));//해당 댓글 번호 받기
			EventReplyDao eventReplyDao = new EventReplyDao();
			EventReplyDto eventReplyDto = eventReplyDao.get(eventReplyIdx);
			...
			if(eventReplyDto==null) {
				...
				UsersDto usersDto = usersDao.get(eventReplyDto.getUsersIdx()); //전달 받은 댓글 번호에 있는 정보를 통해 회원의 정보 추출
